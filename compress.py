...
def parse_opt(known=False):
    ...
    parser.add_argument('--compression', default='backbone', type=str, choices=['backbone', 'global'], help='To compress which part? backbone or all layers?')
    ...
    if opt.compression.lower() == 'backbone':
        ...
        for i in range(10):
            if i not in [1, 2, 3, 4, 5, 6, 7, 8]:
                opt.part.append(f'model.{i}.')
    else:
        opt.part = [f'model.{i}.' for i in range(24)]
        # opt.part = []
        # for i in range(36):
        #     if i not in [1, 2, 3, 4, 5, 6, 7, 8, 10, 19, 20, 23, 25, 26, 28, 30, 31, 33, 35]:
        #         opt.part.append(f'model.{i}.')
    main(opt)
