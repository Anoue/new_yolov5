...
def group_l1prune(model, group, compress_rate, round_to=1):
    ...
    #     if type(filters) != float:
    #         mean_abs_weights = torch.mean(torch.abs(layers.weight), dim=(1, 2, 3))
    #         filters += mean_abs_weights
    #     else:
    #         mean_abs_weights = torch.mean(torch.abs(layers.weight), dim=(1, 2, 3))
    #         mean_abs_weights = F.pad(mean_abs_weights, (0, filters.size(0) - mean_abs_weights.size(0)))
    #         filters += mean_abs_weights
    filters = 0.
    filter_size = None
    for model_id in group:
        ...
        if filter_size is None:
            # 在第一次迭代中，记录下filter的大小
            filter_size = mean_abs_weights.size(0)
            filters += mean_abs_weights
        else:
            print(filter_size, mean_abs_weights.size(0))
            # 在之后的迭代中，如果需要，对mean_abs_weights进行填充
            if mean_abs_weights.size(0) < filter_size:
                mean_abs_weights = F.pad(mean_abs_weights, (0, filter_size - mean_abs_weights.size(0)))
            # else:
            #     filters = F.pad(filters, (0, mean_abs_weights.size(0) - filter_size))
            filters += mean_abs_weights
