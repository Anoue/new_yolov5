...
class Sensitivity(object):
    ...
    def set_group(self, model):
        # back_bottleneck_index = [2, 4, 6, 8, 19, 23, 25, 28, 30, 33, 35]

        # self.groups = []
        # for i in back_bottleneck_index:
        #     group = []
        #     group.append(f'model[{i}].cv2.conv')
    ...
    def __call__(self, model, dataloader, part, sensitivity=None):
        ...
        for k, v in model.named_modules():
            if hasattr(v, 'weight') and not isinstance(v, torch.nn.BatchNorm2d) and any([k.startswith(p) for p in part]) and k not in sensitivity.keys():
                ...
                # import thop
                # p = next(pruned_model.parameters())
                # stride = max(int(model.stride.max()), 32) if hasattr(model, 'stride') else 32  # max stride
                # im = torch.empty((1, p.shape[1], stride, stride), device=p.device)  # input image in BCHW format
                # flops = thop.profile(pruned_model, inputs=(im, ), verbose=False)[0] / 1E9 * 2  # stride GFLOPs
                # print(flops)
